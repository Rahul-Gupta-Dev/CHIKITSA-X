  ...
  return (
    <div className="modal-overlay">
      <div className="modal-content" style={{ maxWidth: '600px' }}>
        ...
        <div style={{ background: 'rgba(15, 23, 42, 0.7)', border: '1px solid rgba(0, 180, 216, 0.3)', borderRadius: '12px', padding: '12px', marginBottom: '20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <img src={selectedHospital.image} alt={selectedHospital.name} style={{ width: '50px', height: '50px', borderRadius: '10px', objectFit: 'cover' }} />
          <div>
            <div style={{ color: '#FFF', fontWeight: 600 }}>{selectedHospital.name}</div>
          ...
          <div>
            ...
            >
              {selectedHospital.specialties.map((spec, i) => (
                <option key={i} value={spec} style={{ background: '#1C2541' }}>{spec}</option>
              ))}
            </select>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              {doctors.map((doc) => (
                <div
                  ...
                  style={{
                    background: selectedDocId === doc.id ? 'rgba(0, 180, 216, 0.15)' : 'rgba(15, 23, 42, 0.5)',
                    border: `1px solid ${selectedDocId === doc.id ? '#00B4D8' : 'rgba(255,255,255,0.1)'}`,
                    borderRadius: '10px',
                    padding: '10px 14px',
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div>
                      <div style={{ color: '#FFF', fontWeight: 600, fontSize: '0.9rem' }}>{doc.name}</div>
                      <div style={{ color: '#94A3B8', fontSize: '0.78rem' }}>{doc.specialty} • {doc.qualification}</div>
                    </div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ color: '#10B981', fontWeight: 600, fontSize: '0.88rem' }}>₹{doc.consultationFee}</div>
                    <div style={{ color: '#FBBF24', fontSize: '0.75rem' }}>★ {doc.rating}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
            <div>
              ...
              <select value={appDate} onChange={(e) => setAppDate(e.target.value)} className="form-control">
                <option value="Today (09 Sep 2026)" style={{ background: '#1C2541' }}>Today (09 Sep 2026)</option>
                <option value="Tomorrow (10 Sep 2026)" style={{ background: '#1C2541' }}>Tomorrow (10 Sep 2026)</option>
                <option value="Thursday (11 Sep 2026)" style={{ background: '#1C2541' }}>Thursday (11 Sep 2026)</option>
              </select>
            </div>
            ...
            <div>
              ...
              <select value={timeSlot} onChange={(e) => setTimeSlot(e.target.value)} className="form-control">
                {currentDoctor?.availableSlots.map((slot, idx) => (
                  <option key={idx} value={slot} style={{ background: '#1C2541' }}>{slot}</option>
                ))}
              </select>
            </div>
          </div>
